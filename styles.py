...
STYLE_BLUEPRINTS = {
    ...
    "style_colortype": {
        ...
        "description": (
            ...
            "Surface color range — stay WITHIN this range by default, don't wander outside it: "
            "white, milk/cream, grey (light or dark), beige/sand, black, and soft pastel pink; if the surface "
            "is a table/wood surface, use light or dark warm brown wood tones. Only step outside this range "
            "(e.g. into a blue or red surface tone) when the panties' own color/undertone genuinely calls for "
            "it as part of the chosen color relationship — never pick an unrelated bright or saturated surface "
            "color just for novelty.\n"
            "Surfaces to vary: choose surface material (silk, linen, marble, wood, velvet, paper, etc.) freely, "
            "but keep its COLOR within the range above (or the justified exception).\n"
            "Props to vary: this style explicitly calls for MORE than the usual minimal prop count — use "
            "2-3 different coordinated prop TYPES together as one styled vignette (e.g. flowers + a "
            "magazine, jewelry + a small purse, a candle + dried flowers, pearls + a book), not just a "
            "single item. Their color should intentionally continue or purposefully contrast the chosen "
            "palette — nothing decorative that doesn't serve the color story. Also match prop TYPE to the "
            "fabric's character, not just its color: delicate/soft fabrics (cotton, jersey, simple lace) "
            "call for softer, simpler props (ribbon, small flowers, a thin delicate chain) rather than "
            "heavy opulent pieces; luxe fabrics (silk, satin, fine lace) can carry more opulent props "
